...
class CLIGraphVisualizer(object):
    ...
    def _visualize_package(self, package, depth, resolved_graph):
    ...
    def visualize_dependency_graph(self, resolved_graph):
        """ prints resolved dependency graph to stdout.

        Args:
            resolved_graph (_type_): A resolved package dependency graph
        """
        for package in sorted(resolved_graph.keys()):
